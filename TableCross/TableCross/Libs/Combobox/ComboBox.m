...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _comboBoxTableView = [[UITableView alloc]initWithFrame:CGRectMake(self.view.frame.origin.x + 3, _button.frame.origin.y+_button.frame.size.height + 20, self.view.frame.size.width - 6, 0)];
        [_comboBoxTableView setSeparatorColor:[UIColor blackColor]];
        
        [_comboBoxTableView setBackgroundColor:[UIColor clearColor]];
        if(IS_IOS7)
        ...
        _imgBackground =[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg_select_popup"]];
        _imgBackground.frame = CGRectMake(self.view.frame.origin.x-1, _button.frame.origin.y+_button.frame.size.height , self.view.frame.size.width+2, 0);
...
-(void)openComboBoxWithAnimation:(UITableView *)comboBoxTableView{
    [UIView animateWithDuration:0.25
                     animations:^(void){
                         [_comboBoxTableView setFrame:CGRectMake(self.view.frame.origin.x + 3, _button.frame.origin.y+_button.frame.size.height + 20, self.frame.size.width - 6, defaultComboBoxTableSize.height)];
                         [_imgBackground setFrame:CGRectMake(self.view.frame.origin.x, _button.frame.origin.y+_button.frame.size.height+ 5, self.frame.size.width, defaultComboBoxTableSize.height + 20)];
                         CGRect frame = self.frame;
                         frame.size.height = _button.frame.size.height+_comboBoxTableView.frame.size.height+30;
                         self.frame = frame;
...
-(void)closeComboBoxWithAnimation:(UITableView *)comboBoxTableView{
    [UIView animateWithDuration:0.25
                     animations:^(void){
                         [_comboBoxTableView setFrame:CGRectMake(self.view.frame.origin.x + 3, _button.frame.origin.y+_button.frame.size.height + 20, self.frame.size.width - 6, 0)];
                         [_imgBackground setFrame:CGRectMake(self.view.frame.origin.x, _button.frame.origin.y+_button.frame.size.height + 20, self.frame.size.width, 0)];
                         CGRect frame = self.frame;
                         frame.size.height = _button.frame.size.height+_comboBoxTableView.frame.size.height + 30;
                         self.frame = frame;
                         frame = self.view.frame;
